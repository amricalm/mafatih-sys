                            ...
                            <tbody>
                                @foreach($menu as $key => $value)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $value->menu }}</td>
                                        <td>{{ $value->slug }}</td>
                                        <td><i class="{{ $value->menu_icon }}"></i></td>
                                        <td>{!! ($value->is_publish == '1') ? '<i class="fa fa-check-square"></i>' : '<i class="fa fa-square"></i>' !!}</td>
                                        <td class="text-right">
                                    ...
                                    <tr>
                                        <td colspan="6">
                                            ...
                                            <table class="table">
                                                ...
                                                @if ($value1['menu_parent']==$value['id'])
                                                <tr>
                                                    <td>{{ $value1->urutan }}</td>
                                                    <td>{{ $value1->menu }}</td>
                                                    <td>{{ $value1->slug }}</td>
                                                    <td><i class="{{ $value1->menu_icon }}"></i></td>
                                                    <td>{!! ($value->is_publish == '1') ? '<i class="fa fa-check-square"></i>' : '<i class="fa fa-square"></i>' !!}</td>
                                                    <td class="text-right">
                                                        <a href="javascript:edit({{ $value1->id }})" class="btn btn-warning btn-sm text-white" id="btnEdit" title="Edit {{ $value->name }}"><i class="fas fa-pen"></i></a>
                                                        <a href="javascript:hapus({{ $value1->id }})" class="btn btn-default btn-sm text-white" onclick="return confirm('Yakin akan dihapus?')" id="btnDelete" title="Hapus {{ $value->name }}"><i class="fas fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                                @endif
                                                @endforeach
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="labelTambahMenu">Tambah Menu</h5>
            ...
            <div class="modal-body">
                <form id="formmenu">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control form-control-sm" id="urutan" name="urutan" placeholder="" style="width:100px;">
                    </div>
                    <div class="form-group">
                        <label for="menu">Nama Menu</label>
                        <input type="text" class="form-control form-control-sm" id="menu" name="menu" placeholder="Nama Menu">
                    </div>
                    ...
                    <div class="form-group">
                        <label for="menu_parent">Menu Induk</label>
                        <select class="form-control form-control-sm" id="menu_parent" name="menu_parent">
                            <option value="0"> - Pilih Salah Satu - </option>
                            @php
                                $level1 = collect($menu)->where('menu_parent','0');
                                foreach($level1 as $k=>$v)
                                {
                                    echo '<option value="'.$v->id.'">'.$v->menu.'</option>';
                                }
                            @endphp
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-sm" name="menu_icon" id="menu_icon">
                        <small>bisa dilihat daftar icon : <a href="https://fontawesome.com/v5/search?m=free" target="_blank">fontawesome.com</a> </small>
                    </div>
                    <div class="form-group">
                        <label for="menu_level">Level</label>
                        <input type="text" name="menu_level" id="menu_level" class="form-control form-control-sm" value="0">
                    </div>
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" name="is_active" id="is_active" value="1" checked="checked">
...
<script>
    ...
    function edit(id)
    {
        ...
        $.post('{{ url('menu') }}/'+id+'/load',{"_token": "{{ csrf_token() }}"},function(data){
            ...
            $("#menu_parent option[value="+data.menu_parent+"]").attr('selected','selected');
            $('#tambahMenu').modal('show');
        })
    }
    function hapus(id)
    {
        $.post('{{ url('menu') }}/'+id+'/hapus',{"_token": "{{ csrf_token() }}"},function(data){
            if(data=='Berhasil')
            {
                location.reload();
            }
    ...
    $('#btnsave').on('click',function(){
        ...
        $.post('{{ url('menu/simpan') }}',{"_token": "{{ csrf_token() }}",'data':frm},function(data){
            if(data=='Berhasil')
            {
                location.reload();
            }
